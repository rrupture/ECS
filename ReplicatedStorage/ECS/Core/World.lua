...
local Catalog = require(script.Parent.Parent.Util.Catalog)
local NetworkBridge = require(script.Parent.Parent.Net.NetworkBridge)
...
function World:_filter(f: any): any
	...
	return {
		kind = if f.kind == "all" and #names > 1 and f.components[1] and f.components[1].isWildcard then "any" else f.kind,
		components = names,
	}
end
...
function World:EventSystem(eventName: string, fn: (...any) -> (), options: any?)
	local commands = self._cmds
	local events = self._events

	local function run(dt: number)
		local queue = events:Drain(eventName)
		for index = 1, #queue do
			local packet = queue[index]
			local argc = packet.n
			local args = table.create(argc + 2)

			for i = 1, argc do
				args[i] = packet[i]
			end
			args[argc + 1] = dt
			args[argc + 2] = commands

			fn(table.unpack(args, 1, argc + 2))
		end
	end
...
function World:On(name: string, fn: (...unknown) -> ()): any
	return self._events:On(name, fn)
end

function World:Once(name: string, fn: (...unknown) -> ()): any
	return self._events:Once(name, fn)
end
...
function World:Queue(name: string, ...: unknown)
	self._events:Queue(name, ...)
end

function World:Drain(name: string): { any }
	return self._events:Drain(name)
end

function World:Pending(name: string): number
	return self._events:Pending(name)
end

function World:OnAdd(component: any, fn: (number, unknown) -> ()): any
	return self._events:OnAdded(self:_name(component), fn)
end

function World:OnChange(component: any, fn: (number, unknown, unknown) -> ()): any
	return self._events:OnChanged(self:_name(component), fn)
end

function World:OnRemove(component: any, fn: (number, unknown) -> ()): any
	return self._events:OnRemoved(self:_name(component), fn)
end

function World:OnComponentAdded(component: any, fn: (number, unknown) -> ()): any
	return self:OnAdd(component, fn)
end

function World:OnComponentRemoved(component: any, fn: (number) -> ()): any
	return self._events:OnRemoved(self:_name(component), function(id)
		fn(id)
	end)
end

function World:Catalog(name: string, entries: any?): any
	local existing = self._catalogs[name]
	if existing then
		if entries then
			existing:RegisterAll(entries)
		end
		return existing
	end

	local catalog = Catalog.new(name, entries)
	self._catalogs[name] = catalog
	return catalog
end

function World:Dispatch(catalogName: string, key: string, ...: unknown): unknown
	local catalog = self._catalogs[catalogName]
	assert(catalog, "World: unknown catalog " .. catalogName)
	return catalog:Run(key, self, ...)
end

function World:CanDispatch(catalogName: string, key: string): boolean
	local catalog = self._catalogs[catalogName]
	return catalog ~= nil and catalog:Has(key)
end

function World:Network(options: any?): any
	if not self._network then
		self._network = NetworkBridge.new(self, options)
	end
