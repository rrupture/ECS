...
function CommandBuffer:Emit(name: string, ...: unknown)
	self._queue[#self._queue + 1] = {
		op = "emit",
		name = name,
		args = { ... },
...
function CommandBuffer:Queue(name: string, ...: unknown)
	self._queue[#self._queue + 1] = {
		op = "queue",
		name = name,
		args = { ... },
...
function CommandBuffer:Flush()
	...
	for i = 1, #queue do
		...
		elseif op == "emit" then
			world:Emit(cmd.name, table.unpack(cmd.args))
		elseif op == "queue" then
			world:Queue(cmd.name, table.unpack(cmd.args))
		end
	end
